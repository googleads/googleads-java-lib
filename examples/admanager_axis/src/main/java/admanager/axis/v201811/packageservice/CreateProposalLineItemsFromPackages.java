...
package admanager.axis.v201811.packageservice;
...
import com.google.api.ads.admanager.axis.v201811.Package;
import com.google.api.ads.admanager.axis.v201811.PackagePage;
import com.google.api.ads.admanager.axis.v201811.PackageServiceInterface;
...
/**
 * This example creates all proposal line items within an IN_PROGRESS package. To determine which
 * packages exist, run GetAllPackages.java.
 *
...
public class CreateProposalLineItemsFromPackages {
  ...
  private static class CreateProposalLineItemsFromPackagesParams extends CodeSampleParams {
    @Parameter(
        names = ArgumentNames.PACKAGE_ID,
        required = true,
  ...
  public static void runExample(
    ...
    System.out.printf(
        "Package with ID %d will create proposal line items using"
            + " product package with ID %d.%n",
        pkg.getId(), pkg.getProductPackageId());

    // Remove limit and offset from statement.
    statementBuilder.removeLimitAndOffset();

    // Create action to activate packages.
    com.google.api.ads.admanager.axis.v201811.CreateProposalLineItemsFromPackages action =
        new com.google.api.ads.admanager.axis.v201811.CreateProposalLineItemsFromPackages();
    ...
    if (result != null && result.getNumChanges() > 0) {
      System.out.printf(
          "Number of packages proposal line items were created for: %d%n", result.getNumChanges());
    } else {
