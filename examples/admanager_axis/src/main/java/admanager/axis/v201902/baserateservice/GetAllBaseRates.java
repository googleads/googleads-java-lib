 ...
 * To create product template base rates, run CreateProductTemplateBaseRates.java. Credentials and
 * properties in {@code fromFile()} are pulled from the "ads.properties" file. See README for more
 * info.
 */
    ...
    do {
      ...
      if (page.getResults() != null) {
        ...
        for (BaseRate baseRate : page.getResults()) {
          System.out.printf(
              "%d) Base rate with ID %d and type '%s',"
                  + " belonging to rate card ID %d was found.%n",
              i++, baseRate.getId(), baseRate.getClass().getSimpleName(), baseRate.getRateCardId());
        }
      }
