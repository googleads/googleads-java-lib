...
/**
 * Verifies that we correctly wire in all properties files to our {@link AdManagerLibConfiguration}.
...
public class AdManagerLibConfigurationIntegrationTest {
  ...
  public void testPropertyFileIncludedInConfig() throws Exception {
    ...
    AdManagerLibConfiguration config =
        Guice.createInjector(new AdManagerModule()).getInstance(AdManagerLibConfiguration.class);
  ...
  private File createPropertiesFile() throws IOException {
    ...
    fileWriter.write("api.admanager.refreshOAuth2Token=false\n");
    fileWriter.flush();
    fileWriter.close();
