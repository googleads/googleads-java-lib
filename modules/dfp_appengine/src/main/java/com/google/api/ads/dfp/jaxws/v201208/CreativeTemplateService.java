
package com.google.api.ads.dfp.jaxws.v201208;

import java.net.MalformedURLException;
import java.net.URL;
import javax.xml.namespace.QName;
import javax.xml.ws.Service;
import javax.xml.ws.WebEndpoint;
import javax.xml.ws.WebServiceClient;
import javax.xml.ws.WebServiceException;
import javax.xml.ws.WebServiceFeature;


/**
 * This class was generated by the JAX-WS RI.
 * JAX-WS RI 2.2.4-b01
 * Generated source version: 2.1
 * 
 */
@WebServiceClient(name = "CreativeTemplateService", targetNamespace = "https://www.google.com/apis/ads/publisher/v201208", wsdlLocation = "https://www.google.com/apis/ads/publisher/v201208/CreativeTemplateService?wsdl")
public class CreativeTemplateService
    extends Service
{

    private final static URL CREATIVETEMPLATESERVICE_WSDL_LOCATION;
    private final static WebServiceException CREATIVETEMPLATESERVICE_EXCEPTION;
    private final static QName CREATIVETEMPLATESERVICE_QNAME = new QName("https://www.google.com/apis/ads/publisher/v201208", "CreativeTemplateService");

    static {
        URL url = null;
        WebServiceException e = null;
        try {
            url = new URL("https://www.google.com/apis/ads/publisher/v201208/CreativeTemplateService?wsdl");
        } catch (MalformedURLException ex) {
            e = new WebServiceException(ex);
        }
        CREATIVETEMPLATESERVICE_WSDL_LOCATION = url;
        CREATIVETEMPLATESERVICE_EXCEPTION = e;
    }

    public CreativeTemplateService() {
        super(__getWsdlLocation(), CREATIVETEMPLATESERVICE_QNAME);
    }

    public CreativeTemplateService(URL wsdlLocation, QName serviceName) {
        super(wsdlLocation, serviceName);
    }

    /**
     * 
     * @return
     *     returns CreativeTemplateServiceInterface
     */
    @WebEndpoint(name = "CreativeTemplateServiceInterfacePort")
    public CreativeTemplateServiceInterface getCreativeTemplateServiceInterfacePort() {
        return super.getPort(new QName("https://www.google.com/apis/ads/publisher/v201208", "CreativeTemplateServiceInterfacePort"), CreativeTemplateServiceInterface.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns CreativeTemplateServiceInterface
     */
    @WebEndpoint(name = "CreativeTemplateServiceInterfacePort")
    public CreativeTemplateServiceInterface getCreativeTemplateServiceInterfacePort(WebServiceFeature... features) {
        return super.getPort(new QName("https://www.google.com/apis/ads/publisher/v201208", "CreativeTemplateServiceInterfacePort"), CreativeTemplateServiceInterface.class, features);
    }

    private static URL __getWsdlLocation() {
        if (CREATIVETEMPLATESERVICE_EXCEPTION!= null) {
            throw CREATIVETEMPLATESERVICE_EXCEPTION;
        }
        return CREATIVETEMPLATESERVICE_WSDL_LOCATION;
    }

}
