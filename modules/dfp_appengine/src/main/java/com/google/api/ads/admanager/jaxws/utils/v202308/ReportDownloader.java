...
package com.google.api.ads.admanager.jaxws.utils.v202308;

import static org.apache.commons.lang.CharEncoding.UTF_8;

import com.google.api.ads.admanager.jaxws.v202308.ApiException_Exception;
import com.google.api.ads.admanager.jaxws.v202308.ExportFormat;
import com.google.api.ads.admanager.jaxws.v202308.ReportDownloadOptions;
import com.google.api.ads.admanager.jaxws.v202308.ReportJobStatus;
import com.google.api.ads.admanager.jaxws.v202308.ReportServiceInterface;
 ...
 *
 * <p>There are two main functions of this class:
 *
 * <ul>
 *   <li>To download a report in Gzip format to a file or any {@code OutputStream}
 *   <li>To get the report and perform a number of tasks on it
 * </ul>
 *
 * <p>{@code ReportUtils} also provides the method {@link #whenReportReady(ReportCallback)} to wait
...
public class ReportDownloader {
  ...
  /**
   * Constructs a {@code ReportDownloader} object for a {@link ReportServiceInterface} and a report
   ...
   *
   * <ul>
   *   <li>{@link ReportCallback#onSuccess()} for a successful scheduling
   *   <li>{@link ReportCallback#onFailure()} for a failed scheduling due to a {@link
   *       ReportJobStatus#FAILED}
   *   <li>{@link ReportCallback#onInterruption()} if the wait thread is interrupted
   *   <li>{@link ReportCallback#onException(Exception)} if there was an exception while waiting for
   *       the report to finish
   * </ul>
   *
   * @param callback the {@code ReportCallback} to call when the job has finished, successfully or
   *     otherwise
   * @throws IllegalArgumentException if {@code callback == null}
   * @return the thread created that handles waiting for the report. {@link Thread#interrupt()} can
   *     be called on the returned thread to interrupt it.
   */
  public Thread whenReportReady(final ReportCallback callback) {
    ...
    Thread waitThread =
        new Thread("ReportUtils.whenReportReady " + reportJobId) {
          @Override
          public void run() {
            try {
   ...
   */
  public URL getDownloadUrl(ReportDownloadOptions options)
      throws ApiException_Exception, MalformedURLException {
    ReportJobStatus status = reportService.getReportJobStatus(reportJobId);
   ...
   * com.google.api.ads.admanager.jaxws.v202308.ExportFormat#CSV_DUMP}.
   *
   * @param options the options to download the report with
